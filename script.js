...
function switchStarsTab(tab) {
  ...
  document.getElementById('stars-content-' + tab).style.display = 'flex';
  document.getElementById('stars-content-' + tab).style.flexDirection = 'column';
...
async function buyStars() {
  try {
    ...
    const tg = window.Telegram.WebApp;
    const user = tg.initDataUnsafe?.user;
    if (!user) { alert('Откройте приложение через Telegram'); return; }
    const res = await fetch('https://fastdrop-payment.prorpo849.workers.dev/create-invoice', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ amount, userId: user.id, username: user.username || user.first_name })
    });
    ...
    alert('Ошибка оплаты. Откройте приложение через Telegram.');
  }
}
...
function activatePromo() {
  ...
  if (!code) { result.style.color = '#f87171'; result.textContent = '❌ Введите промокод!'; return; }
  if (localStorage.getItem(usedKey)) { result.style.color = '#f87171'; result.textContent = '❌ Промокод уже использован!'; return; }
  if (PROMO_CODES[code]) {
    const stars = PROMO_CODES[code];
    setBalance(getBalance() + stars);
    localStorage.setItem(usedKey, 'true');
    result.style.color = '#4ade80';
    result.textContent = `✅ Активировано! +${stars} ⭐️`;
  } else {
...
setTimeout(() => document.getElementById('ton-stars-input').addEventListener('input', function() {
  const stars = parseInt(this.value) || 0;
  const ton = (stars / 90).toFixed(2);
  document.getElementById('ton-amount-display').textContent = `≈ ${ton} TON`;
}), 500);
...
function payTON() {
  ...
  try {
    window.Telegram.WebApp.openTelegramLink(`https://t.me/wallet?startattach=ton_transfer_${address}_${ton}`);
  } catch(e) {
